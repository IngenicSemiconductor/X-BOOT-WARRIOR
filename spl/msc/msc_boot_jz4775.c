...
void spl_main(void)
{
...
#if 1
	{
		...
		__asm__ __volatile__ (
			...
			:"=r"(errorpc)
			:);
		serial_puts_info("Reset errorpc: 0x");
	...
	* (volatile unsigned *) 0xb00000d0 = 0x1;
...
#ifndef CONFIG_FPGA
#if defined(CONFIG_MSC_CLKSRC_MPLL)
	clk = __cpm_get_pllout2();
	msc_cdr = CPM_MSCCDR_MPCS_MPLL | CPM_MSCCDR_CE;
#else
	clk = __cpm_get_cclk();
	msc_cdr = CPM_MSCCDR_MPCS_SRC | CPM_MSCCDR_CE;
#endif
	msc_cdr |= clk % (25000000 / 2) ? clk / 25000000 / 2 : clk / 25000000 / 2 - 1;
	...
	serial_puts_info("Starting X-Boot ...\n");
