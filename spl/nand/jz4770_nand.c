	...
	for (i = 0; i < ecc_count; i++) {
		...
		if (stat & BCH_INTS_ERR) {
			if (stat & BCH_INTS_UNCOR) {
				...
				errs = (u32 *)BCH_ERR0;
				for (i = 0; i < errcnt; i++) {
					/* errs[i>>1] get the BCH error report register value, i-error bit index,
					 * so errs[i>>1]>>((i%2)<<4) means that if the error bit index is odd, 
					 * errs[i>>1] >> 16. ## Please refer to Jz4770 Programming Manual.##
					 */
					 bch_correct(data_buf, (errs[i>>1] >> ((i % 2) << 4)) & BCH_ERR_INDEX_MASK);
				}
			}
		}
...
int *nand_load(int offs, int xboot_size, unsigned char *dst)
{
	...
	for (i=0; i<desc_count; i++) {
		...
		if (desc->image_flag == XBOOT_SECOND_BOOT || desc->image_flag == XBOOT_THIRD_BOOT) {
			...
			page_count=0;
		   	while( page_count < desc->image_size/page_size ){			  				
			   	if ( 0 == page % page_per_block ){  //initial invalid block :last page in each block
				...
				nand_read_page(page+desc->image_offset , (runaddress_back+(page_size-512)), oob_buf);
