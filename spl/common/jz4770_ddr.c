...
#ifdef DEBUG
void ddr_enter_selfrefresh(void)
{
	/* exter self-fresh */
	REG_DDRC_CTRL |= DDRC_CTRL_SR;
	while(!(REG_DDRC_ST & DDRC_ST_SREF));
	/* stop DDR clock */
	REG_CPM_CLKGR0 |= CPM_CLKGR0_DDR;
	while(!(REG_CPM_CLKGR0 & CPM_CLKGR0_DDR));
}

void ddr_exit_selfrefresh(void)
{
	/* start DDR clock */
	REG_CPM_CLKGR0 &= ~CPM_CLKGR0_DDR;
	while(REG_CPM_CLKGR0 & CPM_CLKGR0_DDR);
