...
/*
 *  jz4780_urboard_special -- define special functions of jz4780 test board.
 *
 */
...
{
#if 0
	serial_puts_info("urboard board_lcd_board_init\n");
	set_lcd_power_on();
	__gpio_as_lcd_24bit();
#endif
}
...
{
	__gpio_as_output(32 * 5 + 15);
#ifdef	CONFIG_MSC_X_BOOT
	serial_puts_info("urboard is burning image\n");
	__gpio_clear_pin(32 * 5 + 15);
#else
	__gpio_set_pin(32 * 5 + 15);
#endif
...
{
	select_led();
	serial_puts_info("urboard low i2c pin\n");
	...
	__gpio_as_output(GPIO_I2C2_SCL);
	__gpio_clear_pin(GPIO_I2C2_SCL);

	__gpio_as_output(GPIO_I2C4_SCL);
	__gpio_clear_pin(GPIO_I2C4_SCL);

#if 0
#if 1
#if 0
	//if the board is old urboard:aa, you can open the if condition
