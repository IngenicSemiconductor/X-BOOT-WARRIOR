...
//	int audioOn = 0;  //lltang audio on
	...
	api_mCurrentState = API_HPD; /* default: hot plug detected */

	if (video == NULL)
	{
		error_Set(ERR_HPD_LOST);
		serial_puts_info("video params invalid\n");
		return FALSE;
	}
	...
	{
		error_Set(ERR_HPD_LOST);
		serial_puts_info("cable not connected\n");
		return FALSE;
	}
	...
	do
	{
		...
		{
			 serial_puts_info("DVI mode selected: packets not configured\n");
		}
		if (hdcpOn == TRUE)
