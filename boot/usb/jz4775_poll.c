...
void do_cmd_flash(USB_STATUS *status)
{
	...
	if (0 == strcmp(ptn->name,"system")){
		...
		serial_puts_msg("convert system.img to system.bin starte: ");
		dump_uint(rx_length);
...
void do_cmd_erase(USB_STATUS *status)
{
	...
	if (0 == ptn) {
		...
		if(strncmp(ptn->name,'boot',sizeof(ptn->name)) && strncmp(ptn->name,'recovery',sizeof(ptn->name))){
			serial_puts_msg("update error partition in jz4775_poll.c: ");
...
void jz4775_handle_fastboot(USB_STATUS *status)
{
	if (Bulk_Out_Size) {
...
void jz4775_usb_poll(USB_STATUS *status)
{
	u32 intsts;
...
#if 0
	serial_print("data_buf  = 0x",data_buf,'X');
	serial_print("data_size = 0x",data_size,'X');
	serial_print("kernel_size  = 0x",bootimginfo.kernel_size,'X');
	serial_print("ramdisk_size = 0x",bootimginfo.ramdisk_size,'X');
	serial_print("page_size    = 0x",bootimginfo.page_size,'X');
#endif
...
#if 0
	serial_print("kernel_actual   = 0x",kernel_actual,'X');
	serial_print("kernel_addr     = 0x",bootimginfo.kernel_addr,'X');
	serial_print("ramdisk_actual  = 0x",ramdisk_actual,'X');
	serial_print("ramdisk_addr    = 0x",bootimginfo.ramdisk_addr,'X');
#endif
